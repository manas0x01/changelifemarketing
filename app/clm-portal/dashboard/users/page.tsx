...
const UserDrawer = ({ user, onClose }: { user: UserRecord; onClose: () => void }) => {
  ...
  const credentialRows = [
    { label: 'User ID / Username', value: user.userId ?? user.username },
    { label: 'Login Password', value: user.plainPassword },
    { label: 'Transaction Password', value: user.plainTransactionPassword },
  ];
    ...
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 z-50 flex justify-end bg-[#0A6E5A]/30 backdrop-blur-sm" onClick={onClose}>
      <motion.div initial={{ x: '100%' }} animate={{ x: 0 }} exit={{ x: '100%' }} transition={{ type: 'spring', damping: 25, stiffness: 200 }} className="bg-[#FFFFFF] w-full max-w-md h-full overflow-y-auto shadow-2xl" onClick={(e) => e.stopPropagation()}>
        ...
        <div className="px-6 py-6 space-y-6">
          ...
          {/* Credentials Section — highlighted card */}
          <div>
            <h4 className="font-['Roboto'] text-[0.7rem] uppercase tracking-widest text-[#C9A84C] mb-3 flex items-center gap-2">
              <span className="w-8 h-px bg-[#C9A84C]" />Login Credentials
            </h4>
            <div className="bg-[#0A6E5A]/4 border border-[#0A6E5A]/15 rounded p-3 space-y-3">
              {credentialRows.map(({ label, value }) => (
                <div key={label} className="flex items-center justify-between gap-2">
                  <div className="min-w-0">
                    <p className="font-['Roboto'] text-[0.65rem] uppercase tracking-wider text-[#333333]/40">{label}</p>
                    <p className="font-['Roboto'] text-[0.8rem] font-semibold text-[#0A6E5A] truncate">
                      {value || <span className="text-[#333333]/30 font-normal italic">Hashed (not visible)</span>}
                    </p>
                  </div>
                  {value && (
                    <button
                      onClick={() => copyText(String(value), label)}
                      className="shrink-0 px-2 py-1 text-[0.65rem] font-['Roboto'] font-medium text-[#0A6E5A] border border-[#0A6E5A]/25 hover:bg-[#0A6E5A] hover:text-white transition-colors rounded"
                      suppressHydrationWarning={true}
                    >
                      Copy
                    </button>
