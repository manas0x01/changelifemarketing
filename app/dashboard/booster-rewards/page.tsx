  ...
  return (
    <div className="gd-root min-h-screen text-white">
      ...
      <style jsx global>{`
        ...
        .progress-bar-fill {
          background: linear-gradient(90deg, #FFD700 0%, #f0a500 100%);
          height: 100%;
          border-radius: 50px;
      ...
      <div className="max-w-7xl mx-auto px-4 py-8">
        ...
        {loading ? (
          ...
          <div className="glass-panel rounded-3xl p-12 text-center max-w-2xl mx-auto shadow-2xl">
            <h2 className="text-2xl font-bold mb-4" style={{ color: "#FFD700" }}>Unable to Load Rewards</h2>
          ...
          <>
            {!data.isBooster && (
              <div className="booster-notice">
                <h2 className="text-2xl font-bold mb-2" style={{ color: "#FFD700" }}>Booster Status Required</h2>
            ...
            <div className={`grid grid-cols-1 lg:grid-cols-3 gap-8 mb-16 ${!data.isBooster ? 'opacity-50 pointer-events-none' : ''}`}>
              <div className="lg:col-span-2 glass-panel rounded-3xl p-8 relative overflow-hidden group">
                <div className="absolute top-0 right-0 p-8 opacity-10 group-hover:scale-110 transition-transform">
                  <svg className="w-48 h-48 text-[#FFD700]" fill="currentColor" viewBox="0 0 24 24">
                    <path d="M12 2l3.09 6.26L22 9.27l-5 4.87 1.18 6.88L12 17.77l-6.18 3.25L7 14.14 2 9.27l6.91-1.01L12 2z" />
                  </svg>
                </div>
                
                <h3 className="text-slate-400 font-semibold uppercase tracking-widest mb-2" style={{ color: "rgba(255,215,0,0.6)" }}>Current Rank</h3>
                <h2 className="text-5xl font-extrabold mb-6" style={{ color: "#FFD700", textShadow: "0 0 10px rgba(255,215,0,0.3)" }}>{data.awardRankStatus?.rankName || "Booster Member"}</h2>
                ...
                {nextRank ? (
                  ...
                  <div className="border border-amber-500/30 rounded-2xl p-6" style={{ background: "rgba(255,215,0,0.1)" }}>
                    <p className="text-amber-500 font-bold text-xl" style={{ color: "#FFD700" }}>🎉 Ultimate Rank Achieved!</p>
              ...
              <div className="glass-panel rounded-3xl p-8 flex flex-col justify-between items-center text-center">
                 <h3 className="text-slate-400 font-semibold uppercase tracking-widest mb-4" style={{ color: "rgba(255,215,0,0.6)" }}>Latest Award</h3>
                 {data.awardRankStatus?.rank > 0 ? (
                    <div className="animate-float">
                      <img 
                        src={RANKS[data.awardRankStatus.rank - 1].image} 
                        alt="Award" 
                        className="w-48 h-48 object-contain mb-4 drop-shadow-[0_0_30px_rgba(255,215,0,0.45)]"
                      />
                      <h4 className="text-2xl font-bold" style={{ color: "#FFD700" }}>{RANKS[data.awardRankStatus.rank - 1].award}</h4>
                  ...
                  <div key={idx} className={`reward-card rounded-2xl p-6 flex flex-col items-center text-center ${isAchieved ? 'achieved' : ''}`}>
                    <div className="relative mb-6">
                      <img 
                        src={rank.image} 
                        alt={rank.name} 
                        className={`w-32 h-32 object-contain transition-all duration-300 ${!isAchieved ? 'grayscale opacity-25' : 'drop-shadow-[0_0_15px_rgba(255,215,0,0.3)]'}`}
                      />
                      {isAchieved && (
                    ...
                    <span className="text-xs font-bold uppercase tracking-widest mb-1" style={{ color: "rgba(255,215,0,0.5)" }}>Rank {rank.rank}</span>
                    ...
                    <div className="space-y-2 mb-6">
                      <p className="text-slate-300 text-sm">Target: <span className="text-white font-semibold" style={{ color: "#FFD700" }}>{rank.left}-{rank.right} Pairs</span></p>
                      <p className="font-semibold text-sm leading-tight" style={{ color: isAchieved ? "#FFD700" : "rgba(255,215,0,0.8)" }}>{rank.award}</p>
                    </div>
