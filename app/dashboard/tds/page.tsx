...
export default function TDSReportPage() {
  ...
  const handleExportCSV = () => {
    ...
    const rows   = data.map(r =>
      `${r.srNo},"${r.fromDate.split(' ')[0]}",${r.total},${r.tds}`
    ).join("\n");
    const blob = new Blob([header + "\n" + rows], { type: "text/csv" });
    const url  = URL.createObjectURL(blob);
    const a    = document.createElement("a"); a.href = url; a.download = "tds-report.csv"; a.click();
    ...
    <>
      <style>{`
        ...
        * { margin:0; padding:0; box-sizing:border-box; }
        ...
        .num-bold { font-weight:700; color:#fff; }
        .num-tds { color:#ff4444; font-weight:700; }
      ...
      <div className="pp-root" onClick={() => dropdownOpen && setDropdownOpen(false)}>
        ...
        <div className="breadcrumb">
          ...
          <span className="sep">/</span>
          <span className="current">TDS Report</span>
        </div>
        ...
        <div className="page-body">
          <div className="main-card">
            ...
            <div className="total-summary">
              Total TDS Deducted : <span>₹{totalTDS.toLocaleString("en-IN")}</span>
            </div>
                  ...
                  {loading ? (
                    ...
                    data.map((row) => (
                      <tr key={row.srNo}>
                        ...
                        <td className="num-tds">₹{row.tds.toLocaleString("en-IN")}</td>
                      </tr>
                    ))
                  )}
                </tbody>
            ...
            {!loading && (
              <div className="table-footer">
                ...
                <span className="footer-total">Total TDS: ₹{totalTDS.toLocaleString("en-IN")}</span>
              </div>
            )}
          </div>
        </div>
