...
async function diagnose() {
  ...
  if (user.sessionBasedIncome && user.sessionBasedIncome.length > 0) {
    user.sessionBasedIncome.forEach((s: any, i: number) => {
  ...
  if (user.basicIncomeRecords && user.basicIncomeRecords.length > 0) {
    user.basicIncomeRecords.forEach((r: any) => {
      console.log(`  #${r.srNo}:`, {
        amount: r.amount,
        date: r.date,
  ...
  if (downlineResult.length > 0) {
    ...
    descendants.sort((a: any, b: any) => a.depth - b.depth).forEach((d: any) => {
  ...
  if (user.sessionBasedIncome && user.sessionBasedIncome.length > 0) {
    ...
    if (seen.size === user.sessionBasedIncome.length) {
