/**
 * Fix Session-Based Income for CLM821812 - 17th July
 *
 * Target:
 *   17th July MORNING   → 1 pair
 *   17th July EVENING   → 0 pairs (remove if exists)
 *
 * Run: node scratch/fix-clm821812-july17.mjs
 */
...
// Patches to apply
const PATCHES = [
  { date: '2026-07-17', sessionType: 'morning', pairs: 1 },
  { date: '2026-07-17', sessionType: 'evening', pairs: 0 }, // remove
];
...
async function main() {
  ...
  console.log('\n📋 CURRENT 17th July session records:');
  sessions.filter(s => {
      const d = new Date(s.date || s.sessionDate);
      return toISTDateStr(d) === '2026-07-17';
  }).forEach((s, i) => {
      const d = new Date(s.date || s.sessionDate);
