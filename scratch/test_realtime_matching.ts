...
async function run() {
  ...
  const rootId = 'REALTIME_ROOT';
  const leftId = 'REALTIME_LEFT';
  const rightId = 'REALTIME_RIGHT';
  const password = 'change123';

  // 1. Setup
  await User.deleteMany({ username: { $regex: /REALTIME_/ } });
  
  console.log('Creating users...');
  await new User({ username: rootId, userId: rootId, password, transactionPassword: password, isBooster: true, basicRank: 'Booster', registeredPackage: 'BASIC_PLAN' }).save();
  ...
  for (let i = 3; i <= 13; i++) {
    const uL = `REALTIME_L_${i}`;
    const uR = `REALTIME_R_${i}`;
  ...
  if (finalRoot?.boosterMatchingRecords?.[0]) {
      console.log(`Record[0]: Paid=${finalRoot.boosterMatchingRecords[0].paidPairs}, Flashed=${finalRoot.boosterMatchingRecords[0].flashedPairs}`);
  }
